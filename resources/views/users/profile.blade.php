    ...
    <div class="col-12 col-sm-5 px-0 d-flex justify-content-end align-items-center px-1 mb-2">
       <div class="btn-group mr-1 mb-1">
        ...
        <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 40px, 0px);">
            <a class="dropdown-item" href="{{ route('users.view', $user->id) }}">استعراض الملف</a>
            <a class="dropdown-item" href="{{ route('users.edit', $user->id) }}">تعديل الملف</a>
            <div class="dropdown-divider"></div>
            <form action="{{route('users.delete',$user->id)}}" method="post" onsubmit="return confirm('هل أنت متأكد من حذف هذا المستخدم نهائيا و جميع تفاصيله من البرنامج')">
                @csrf
                @method('delete')
            <button class="dropdown-item btn-danger btn" type="submit">حذف المستخدم</button>
            </form>
        </div>
    </div>
    <div class="btn-group mr-1 mb-1">
        <button type="button" class="btn btn-info btn-sm btn-min-width dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> التواصل مع المستخدم</button>
        <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 40px, 0px);">
            @if ( isset($user->customer_mobile))
        <a class="dropdown-item" href="tel:{{$user->customer_mobile}}">اتصال بالموبايل</a>
            @else
            <button class="dropdown-item" href="#">اتصال بالموبايل</button>
            @endif
            @if ( isset($user->customer_phone))
        <a class="dropdown-item" href="tel:{{$user->customer_phone}}">اتصال بالتليفون</a>
            @else
        ...
        <a class="dropdown-item" href="mailto:{{$user->customer_email}}"> ارسال ايميل</a>
            @else
        ...
        <div class="row">
          ...
          <div class="col-xl-6 col-lg-12 mb-1">
            <div class="form-group text-center">
                ...
                <button type="button" class="btn btn-float btn-outline-cyan" style="cursor: context-menu"><i class="">0</i><span>عدد فواتير الشراء</span></button>
            <button type="button" class="btn btn-float btn-float-lg btn-outline-pink"  style="cursor: context-menu"><i class="">0 ج,م</i><span>إجمالي المبالغ من الفواتير</span></button>
                <button type="button" class="btn btn-float btn-outline-cyan"  style="cursor: context-menu"><i class="">0</i><span>عدد عروض الأسعار</span></button>
            </div>
        </div>
        </div>
      </div>
